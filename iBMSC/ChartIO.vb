...
Partial Public Class MainWindow
    Private Sub OpenBMS(ByVal xStrAll As String)
        ...
        Dim xWavKeyLength As Integer = 2
        ...
        For Each sLine In xStrLine
            ...
            ElseIf sLineTrim.StartsWith("#WAV", StringComparison.CurrentCultureIgnoreCase) Then
                Dim xValue As String = Mid(sLineTrim, Len("#WAV") + 1).Trim
                Dim xSeparator As Integer = xValue.IndexOfAny(New Char() {" "c, ControlChars.Tab})
                If xSeparator > 0 Then
                    Dim xIndexText As String = Mid(xValue, 1, xSeparator)
                    Dim xIndex As Integer = C36to10(xIndexText)
                    If xIndexText.Length >= 3 Then xWavKeyLength = 3
                    If xIndex >= 1 AndAlso xIndex <= UBound(hWAV) Then hWAV(xIndex) = Mid(xValue, xSeparator + 2).Trim
                End If
...
AddExpansion:       xExpansion &= sLine & vbCrLf
        ...
        For Each sLine In xStrLine
            ...
            Dim xKeyLength As Integer = IIf(Channel = "03" OrElse Channel = "08" OrElse Channel = "09" OrElse Channel = "SC", 2, xWavKeyLength)
            Dim xKeyZero As String = New String("0"c, xKeyLength)

            For xI1 = 8 To Len(sLineTrim) - xKeyLength + 1 Step xKeyLength   'For all Ks within that line
                Dim xKeyText As String = Mid(sLineTrim, xI1, xKeyLength)
                If xKeyText = xKeyZero Then Continue For 'If the K is not zero
                ...
                With Notes(UBound(Notes))
                    ...
                    .Selected = False
                    .VPosition = MeasureBottom(xMeasure) + MeasureLength(xMeasure) * ((xI1 - 8) / xKeyLength) / ((Len(sLineTrim) - 7) / xKeyLength)
                    .Value = C36to10(xKeyText) * 10000

                    If Channel = "03" Then .Value = Convert.ToInt32(xKeyText, 16) * 10000
                    If Channel = "08" Then .Value = hBPM(C36to10(xKeyText))
                    If Channel = "09" Then .Value = hSTOP(C36to10(xKeyText))
                    If Channel = "SC" Then .Value = hSCROLL(C36to10(xKeyText))
                End With
        ...
        For xI1 = 1 To UBound(hWAV)
            LWAV.Items.Add(C10to36(xI1, 3) & ": " & hWAV(xI1))
        Next
            ...
            For i = 0 To UBound(relativeMeasurePos)        'assign K texts
                If CInt(relativeMeasurePos(i) / xGCD) > UBound(xStrKey) Then
                    ...
                    With xprevNotes(UBound(xprevNotes))
                        ...
                        .Value = C36to10(NoteStrings(i)) * 10000
                    End With
                    ...
                    Continue For
                End If
                If xStrKey(CInt(relativeMeasurePos(i) / xGCD)) <> xKeyZero Then
                    hasOverlapping = True
                End If
...
Jump1:
        ...
        LWAV.Visible = False
        LWAV.Items.Clear()
        For xI1 As Integer = 1 To UBound(hWAV)
            LWAV.Items.Add(C10to36(xI1, 3) & ": " & hWAV(xI1))
        Next
        LWAV.SelectedIndex = 0
        LWAV.Visible = True
    ...
    Private Sub SaveiBMSC(ByVal Path As String)
        ...
        Try
            ...
            For i As Integer = 1 To UBound(hWAV)
                If i <= Short.MaxValue AndAlso hWAV(i) <> "" Then xWAVCount += 1
            Next
            bw.Write(xWAVCount)

            For i As Integer = 1 To UBound(hWAV)
                If i > Short.MaxValue OrElse hWAV(i) = "" Then Continue For
                bw.Write(CShort(i))
                bw.Write(hWAV(i))
            Next
